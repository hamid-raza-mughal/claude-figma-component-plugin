...
**Date:** 2026-07-29 · **Phase 1 model calls: zero.**
...
|---|---|---|
| `Agentic/adalfi-design-curated-tokens.json` | **876,098** | The authoritative curated source. **Contributes zero bytes to any model input by architecture** (P1-FINAL §18). |
| — same file, character count | 876,096 | 2 bytes of multi-byte UTF-8. Recorded because an earlier note carried 876,096 as a *byte* count; the hash is unchanged, so the file is the same and the earlier figure was the character count. |
...
| Full v1 artifact bundle | 2,128,772 | 39 files. Bounds the corpus a future run could accidentally ingest. |
...
|---|---|---|
| Curated JSON ≈ 250,000 tokens | **estimate** | 876,096 chars ÷ ~3.5 chars/token. No tokenizer was run. A real count needs the model's tokenizer, which arrives with the Phase 2 adapter. |
