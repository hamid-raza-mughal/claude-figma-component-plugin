...
|---|---|---|---|
| 1 | **1-hop alias resolution.** The prototype's `float(first)` yields `None` for any `VARIABLE_ALIAS` value. | Measured: 169 of 531 variables hold at least one alias, and **max chain depth is 1**. Numeric coverage rises from **166 → 306** variables. (2026-07-28 export: 169 of 504, 142 → 282. Re-measured against the 2026-09-06 export; the shape of the finding is unchanged, the counts are not.) | The exact-value rule can now fire for records where it previously could not. Depth capped at 4 with cycle detection, so a future deeper or cyclic export degrades to "unresolved" rather than hanging. |
