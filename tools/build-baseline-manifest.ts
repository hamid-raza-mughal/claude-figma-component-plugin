...
const ANNOTATIONS: Readonly<Record<string, Annotation>> = {
  'Agentic/adalfi-design-curated-tokens_latest.json': {
    status: 'active-input',
    type: 'curated design-system export (authoritative source, exported 2026-09-06T12:41Z)',
    defect:
      'All 676 style ids carry a trailing comma inside the string; 0 of 531 variable ids do. ' +
      '`bound_variables` is polymorphic (list[0] ×381, list[1] ×189, list[4] ×1, dict-keyed ×105). ' +
      'Path casing differs by class: variables TitleCase, styles lowercase. ' +
      'A **rename** export, not an additive one: 43 entries added, 13 removed, 123 modified against ' +
      'the 2026-07-28 file. The removals are renames — `sys/dark/bg/*` -> `sys/dark/bgs/*`, and the ' +
      '`radius/round-shape/*` leaves replaced by a level-deeper family plus `lg-scale/*` — which means ' +
      'a retired name can be a strict prefix of a live one. `diagnostics.counts` is self-reported and ' +
      'is cross-checked at ingestion rather than trusted (SOURCE_COUNT_DISAGREEMENT).',
  },
  'Agentic/adalfi-design-curated-tokens.json': {
    status: 'superseded',
    type: 'curated design-system export (2026-07-28 — the historical baseline)',
    defect:
      ...
      'Path casing differs by class: variables TitleCase, styles lowercase. ' +
      'Retained deliberately: `tests/resolver/retired-names.test.ts` reads it to prove the thirteen ' +
      'retired names were real, so "removed" is a measured claim about a rename rather than about a typo. ' +
      'No production path reads it — `CURATED_SOURCE_RELATIVE` points at `_latest.json` (MB-17).',
    successor: 'Agentic/adalfi-design-curated-tokens_latest.json',
  },
...
**Authoritative curated source:** \`Agentic/adalfi-design-curated-tokens_latest.json\` — the 2026-07-28 export beside it is retained as the historical baseline and is read only by the rename-regression suite.
