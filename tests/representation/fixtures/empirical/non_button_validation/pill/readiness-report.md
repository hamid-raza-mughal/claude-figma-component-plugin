...
**Contamination check.** The two baseline exports (`NAME-0001-components-848-3342-basic-tree.json`, `…-terminal.json`) were string-searched for `NODE-0317`, `NODE-0539`, `NODE-0001`, `NODE-0513`, and `Pill`. **All five probes returned false.** The baseline exports contain no Pill data whatsoever. Every component-specific fact in this experiment is therefore re-derived from live Figma reads, not carried over.
...
|---|---|---|
| File key | `FILEKEY-0002` | observed |
| File name | `NAME-0001-Components` | observed |
| Selected node ID | `NODE-0539` | observed |
...
**Is the selection the complete documentation area for one non-Button component?** Yes. `NODE-0539` contains exactly three functional children:

- `NODE-0540` `header` — page chrome (title / subheading).
- `NODE-0001` `components_holder` — the documentation block, headlined **"Pills — Anatomy & Usage"** (`NODE-0003`), containing two theme cards.
- `NODE-0545` `components_holder` — the build holder, containing the five COMPONENT_SETs.

Plus one hidden `labels` frame (`NODE-0546`). No second component's documentation appears inside the selection, and no Pill material was found outside it within the selection's subtree. **Scope is coherent.**

## 3. Non-Button confirmation

Confirmed. The component is **Pill**. The string `Button` appears nowhere in the 650-line structural capture of `NODE-0539`. Component set names are `Pill – Highline`, `Pill – Highlighted`, `Pill – Filled`, `Pill – Text`, `Pill – Pulse Animation`. Headline text is `Pills — Anatomy & Usage`.
...
| `mcp__Figma__get_design_context` | ⚠️ **not pursued** | Returned a Code Connect setup prompt instead of design context for `NODE-0012`. Acting on it would have required a write-scoped mapping action outside this experiment's authority. Recorded as a limitation, not followed. |
...
Corroborating (but not proving) evidence that these are true COMPONENT_SETs: `evidence/screenshot-787-4544-component-sets.png` shows the four visible sets each drawn with Figma's dashed component-set boundary.
...
| L-8 | `get_variable_defs` returned `{}` for hidden set `NODE-0513` | Cannot tell "no bindings" from "not resolved because hidden" | Recorded as `unverified`, **not** as absence |
...
| L-5 | No variable collection name/ID | **closed** — `colors`, `…4c8f51ad…/NODE-0559`, `remote: true`, 105 variables |
| L-6 | No variable mode names/IDs | **closed** — Dark `NODE-0299` (default), Light `NODE-0558` |
